...
export default async function mentorAssignments({ params }: {
  ...
  return (
    ...
    <div className="mx-2 md:mx-6 px-2 md:px-8 py-2 flex flex-col gap-4">
      ...
      {!coursesWithAssignments||coursesWithAssignments.length===0 ? <div className="text-center">No Assignments found!</div> : <StudentWiseAssignments courses={courses} assignments={coursesWithAssignments} userId={params.id} />}
    </div>
  );
}
