...
const UserTable = ({ users, params }: { users: Array<any>, params: any }) => {
    ...
    const mentors = users.filter(user => user.role === 'MENTOR' && user.enrolledUsers.find(({ course }: { course: any }) => course.id === params.id) !== undefined);
                    ...
                    <tr className=' bg-sky-800'>
                        ...
                        <th className="border border-gray-300 px-4 py-2 " >
                            <div className=' flex items-center justify-center'>
                                ...
                                Username &nbsp;
                                {
                                    sortColumn !== 'username' && <FaSort onClick={() => handleSort('username')} className=' cursor-pointer' />
                                }
                                {
                    ...
                    {displayedUsers.map((user, index) => (
                        <tr key={user.id} className={`${user.enrolledUsers.length === 0 ? 'bg-red-500 hover:bg-red-600 ' : 'hover:bg-slate-600'} `}>
                            ...
                            <td className="border border-gray-300 px-4 py-2">
                                {
                                    user.role === 'STUDENT' && user.enrolledUsers.find(({ course }: { course: any }) => course.id === params.id) !== undefined
                                        ? <select value={user.mentor} onChange={(e) => handleMentorChange(user.username, e.target.value)} className="border border-gray-300 px-2 py-1 rounded-md focus:outline-none focus:border-blue-500">
                                            <option value="">None</option>
                                            {mentors.map((mentor) => (
                                                <option key={mentor.id} value={mentor.username} selected={mentor.username === user.enrolledUsers.find(({ course }: { course: any }) => course.id === params.id)?.mentorUsername}>
                                                    {mentor.username}
                                                </option>
                                            ))}
                                        </select>
                            ...
                            <td className="border border-gray-300">
                                {user.enrolledUsers.find(({ course }: { course: any }) => course.id === params.id) === undefined ?
                                    <button disabled={loading} onClick={() => handleEnroll(user.username)} className="px-3 ml-2 py-1.5 flex items-center justify-center bg-emerald-700 text-white rounded-full hover:bg-emerald-800 select-none focus:outline-none focus:bg-emerald-800">
                                        Enroll &nbsp; <FaUserPlus className=' w-6 h-6' />
                                    </button>
                                    :
                                    <button disabled={loading} onClick={() => handleUnenroll(user.username)} className="px-3 ml-2 py-1.5 flex items-center justify-center bg-red-800 text-white rounded-full hover:bg-red-700 select-none focus:outline-none focus:bg-red-800">
