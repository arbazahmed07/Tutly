...
export default function Doubts() {
    ...
    return (
        <div className="h-72 w-full bg-primary-400 p-10 text-secondary-50">
            ...
            <div id="accordion-color" data-accordion="collapse" className="mt-16 bg-primary-500">
            {
               QA.map((qa,index)=>(
                  <div key={index}>
                    <button type="button" className={`flex items-center justify-between w-full p-3 font-medium border gap-3 ${openAccordion === index && 'bg-primary-400: bg-primary-800'}`} onClick={() => toggleAccordion(index)}>
                      <div>{qa.question}</div>
