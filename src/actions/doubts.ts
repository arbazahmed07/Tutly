...
//for mentor/instructor to display all user douts in class tab
...
//get all doubts for mentor/instructor in a course 
...
export const createDoubt = async (classId: string, title?:string,description?:string) => {
    const currentUser = await getCurrentUser();
    if (!currentUser) return null;
