...
import { ChevronLeft, ChevronRight, Clock, Paperclip, Calendar as CalendarIcon } from "lucide-react"
...
export default function Sidebar({ events, onEventSelect, onDateChange, isLoading = false }: SidebarProps) {
  const [searchParams, setSearchParams] = useSearchParams()
  const selectedDate = searchParams.get("date") ? dayjs(searchParams.get("date")).toDate() : new Date()

  const filteredEvents = events.filter(
    event => dayjs(event.startTime).format("YYYY-MM-DD") === dayjs(selectedDate).format("YYYY-MM-DD")
  )
