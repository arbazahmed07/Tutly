  ...
  return (
    <>
      <TooltipProvider>
        <Tooltip delayDuration={50}>
          <TooltipTrigger asChild>
            <div className={`absolute right-2 top-2 h-2 w-2 rounded-full ${isOnline ? "bg-green-500 animate-pulse" : "bg-gray-500"}`} />
          </TooltipTrigger>
          <TooltipContent side="top" align="center" sideOffset={5}>
            <p>
              {isOnline ? "User is currently online!" : `Last seen ${lastSeenTime.fromNow()}`}
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    ...
    <div className="container mx-auto max-w-7xl space-y-8 px-4 py-8">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {currentUsers.map((user) => (
          <Card key={user.id} className="group relative overflow-hidden bg-card transition-all duration-300 hover:scale-[1.02] hover:shadow-lg">
            <CardHeader className="flex flex-row items-center gap-4 p-4">
              <Avatar className="h-12 w-12 ring-2 ring-primary/20 transition-transform duration-300 group-hover:scale-110">
                <AvatarImage src={user.image ?? "/placeholder.jpg"} alt={user.name ?? ""} className="object-cover" />
                <AvatarFallback className="bg-primary/20 text-primary text-sm font-semibold">
                  {user.name?.split(" ").map((n) => n[0]).join("") ?? user.username?.slice(0, 2)}
                </AvatarFallback>
              </Avatar>
              <div className="space-y-0.5">
                <CardTitle className="text-lg font-bold tracking-tight">{user.name ?? user.username}</CardTitle>
              ...
              <div className="space-y-2">
                ...
                <Dialog open={isOpen && selectedUser?.id === user.id} onOpenChange={(open) => {
                  ...
                  <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader>
                      <DialogTitle className="text-xl font-bold">Send Message to {user.name ?? user.username}</DialogTitle>
