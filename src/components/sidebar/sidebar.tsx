        ...
        <div className={`${!menu&&"max-sm:hidden max-sm:pt-12"} p-3 -mt-3 bg-background absolute sm:fixed left-0 sm:top-14 z-40  min-h-dvh px-2 shadow-sm shadow-blue-800/40`}>
            {
                items.map((item)=>{
                    return (
                      <div key={item.path}>
                            <div onClick={()=>Mobile(item.path)} key={item.path} className={`${pathname===item.path?"bg-blue-600 text-white":"hover:bg-blue-500 hover:text-white"} m-auto rounded md:hidden px-4 py-3 my-2 flex items-center gap-4 cursor-pointer`}>
