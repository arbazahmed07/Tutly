...
export default function CourseCard({ course, currentUser }: any) {
  ...
  return (
    <div
      key={course.id}
      className="rounded-lg border m-auto mt-3 w-[280px] md:mx-2"
      style={{
        boxShadow:
    ...
    >
      <div
        className="h-[150px]  relative text-secondary-700 bg-white rounded-t-lg cursor-pointer"
        onClick={
          expired()
            ? () => router.push(`/courses`)
        ...
        {currentUser.role === "INSTRUCTOR" && (
          <div className=" flex items-center justify-between gap-3">
            <Suspense fallback={<Loader />}>
              <button
                onClick={() => router.push(`/courses/${course.id}/allUsers`)}
              >
